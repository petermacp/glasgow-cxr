...
Try calculating correlation between RR.peak and RR.level 
...
cor_rr_peak_rrlevel <- round(cor(rr_peak$RR.peak, rr_level$RR.level), digits = 2)


c <- left_join(rr_peak, rr_level) %>%
  ggplot(aes(y=RR.peak, x = RR.level)) +
  geom_hex() +
  geom_smooth(se=FALSE, colour="firebrick") +
  geom_text(aes(y=2.2, x=0.55, label=cor_rr_peak_rrlevel), colour="firebrick")  +
  scale_fill_gradient(high="lightblue1",low="darkblue") +
  theme_ggdist() +
  theme(legend.position = "none",
        panel.border = element_rect(colour = "grey78", fill=NA))
...
ggsave(here("figures/temp_cors.pdf"))
...
c <- left_join(rr_peak, rr_level) %>%
  ...
  geom_hex() +
  geom_smooth(se=FALSE, colour="firebrick") +
  geom_text(aes(y=3, x=0.4, label=cor_rr_peak_rrlevel), colour="firebrick")  +
...
(plot_spacer() + b) / (a + c) +
  plot_annotation(
  title = 're_formula=sample_new_levels = "uncertainty"')

ggsave(here("figures/temp_cors_2.pdf"))
